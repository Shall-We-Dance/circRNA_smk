...
* `sashimi.plot_rmats_events`: also plot ordinary rMATS events such as `SE.JC`/`A3SS.JC` (`false` by default; BSJ-only plots remain enabled through `sashimi.enabled`)
...
* BSJ sashimi outputs are written under `plots_bsj_only/`
