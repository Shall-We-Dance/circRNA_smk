...
rule star_align_ciri3:
    ...
    params:
        ...
        prefix=lambda wc, output: str(output.chimeric).removesuffix("Chimeric.out.junction")
    shell:
        r"""
        set -euo pipefail
        bam_dir=$(dirname "{output.bam}")
        log_final_dir=$(dirname "{output.log_final}")
        log_final_qc_dir=$(dirname "{output.log_final_qc}")
        log_dir=$(dirname "{log}")
        mkdir -p "$bam_dir" "$log_final_dir" "$log_final_qc_dir" "$log_dir"
...
rule bwa_remap_ciri3:
    ...
    shell:
        r"""
        set -euo pipefail
        output_dir=$(dirname "{output.bwa_bam}")
        log_dir=$(dirname "{log}")
