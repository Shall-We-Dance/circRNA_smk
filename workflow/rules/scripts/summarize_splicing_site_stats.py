...
colors = plt.cm.Blues([0.35 + 0.55 * i / max(len(ratio_bins), 1) for i in range(len(ratio_bins))])
for color, b in zip(colors, ratio_bins):
    values = [ratio_fraction.get(s, {}).get(b, 0.0) for s in samples]
    axes[2].bar(xs, values, bottom=bottom, label=b, color=color)
    bottom = [x + y for x, y in zip(bottom, values)]
axes[2].set_xticks(xs)
axes[2].set_xticklabels(samples, rotation=45, ha="right")
axes[2].set_ylim(0, 1)
axes[2].set_ylabel("fraction")
axes[2].set_title("BSJ/FSJ ratio-bin composition")
axes[2].legend(fontsize=7, title="ratio bin", loc="upper left", bbox_to_anchor=(1.02, 1.0))
