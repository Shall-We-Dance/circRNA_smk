...
rule prepare_dcc_bsj_motif_inputs:
    input:
        bsj=f"{DCC_OUTDIR}/per_sample/{{sample}}.dcc.BSJ_Matrix",
        ciri3=f"{DCC_OUTDIR}/per_sample/{{sample}}.dcc",
        fasta=config["reference"]["fasta"]
    output:
        site_table=f"{DCC_OUTDIR}/motif/{{sample}}/bsj_sites.tsv",
        site_fasta=f"{DCC_OUTDIR}/motif/{{sample}}/bsj_sites.fa"
    log:
        "logs/dcc_motif/{sample}.prepare.log"
    conda:
        "envs/motif.yaml"
    params:
        flank=int(config.get("motif", {}).get("flank", 30)),
...
rule run_homer_dcc_bsj_motif:
    input:
        site_table=f"{DCC_OUTDIR}/motif/{{sample}}/bsj_sites.tsv",
        site_fasta=f"{DCC_OUTDIR}/motif/{{sample}}/bsj_sites.fa"
    output:
        motif_summary=f"{DCC_OUTDIR}/motif/{{sample}}/bsj_motif_summary.tsv",
        homer_dir=directory(f"{DCC_OUTDIR}/motif/{{sample}}/homer"),
        known_results=f"{DCC_OUTDIR}/motif/{{sample}}/homer/knownResults.txt"
    log:
        "logs/dcc_motif/{sample}.homer.log"
    threads: int(config["threads"].get("homer", 1))
    conda:
        "envs/motif.yaml"
    params:
...
rule summarize_dcc_motif_stats:
    input:
        site_tables=expand(f"{DCC_OUTDIR}/motif/{{sample}}/bsj_sites.tsv", sample=SAMPLES),
        known_results=expand(f"{DCC_OUTDIR}/motif/{{sample}}/homer/knownResults.txt", sample=SAMPLES)
    output:
        site_stats=f"{DCC_OUTDIR}/motif/all_samples_site_stats.tsv",
        known_merged=f"{DCC_OUTDIR}/motif/all_samples_known_motif_summary.tsv",
        homer_summary=f"{DCC_OUTDIR}/motif/all_samples_homer_summary.tsv",
        top_motifs=f"{DCC_OUTDIR}/motif/all_samples_top_known_motifs.tsv",
        plot=f"{DCC_OUTDIR}/motif/all_samples_overview.png",
        heatmap=f"{DCC_OUTDIR}/motif/all_samples_known_motif_heatmap.png"
    log:
        "logs/dcc_motif/summarize.log"
    params:
        samples=SAMPLES
    conda:
        "envs/motif.yaml"
